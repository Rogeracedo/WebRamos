...
try {
    $dataRespuesta = array();
    $query = "SELECT f.*,c.*,s.Tipo as servicio, s.ID_Servicio as idservicio FROM formulario f join cliente c on c.ID_Cliente = f.ID_Cliente join servicio s on s.ID_Servicio = f.idServicio where f.ID_Cliente=$id order by f.ID_Formulario DESC; ";
                ...
                <tbody id=" project-list">
                    <?php foreach ($dataRespuesta as $data): ?>
                        <tr>
                            ...
                            <td><?php echo ($data['estado'] == 0) ? "Pendiente" : (($data['estado'] == 1) ? "Aceptado" : "Rechazado"); ?></td>
